...
import { pickRandomFundament, type FundamentEntry } from '../../data/fundament';
import { type Locale, t, getLocaleFromCookie } from '../../lib/i18n';
import { getCookie, setCookie } from '../../lib/cookies';
import { FundamentSymbol } from './FundamentSymbol';
import { FundamentSelectionModal } from './FundamentSelectionModal';

export function FundamentGame({ onBack }: { onBack: () => void }) {
  const [locale, setLocale] = useState<Locale>('en');
  const [entry, setEntry] = useState<FundamentEntry | null>(null);
  ...
  const generateNewCard = useCallback((excludeId?: number) => {
    setEntry(pickRandomFundament(excludeId));
  ...
  useEffect(() => {
    ...
    const saved = getCookie('fundamentBestStreak');
  ...
  const evaluateAnswer = () => {
    ...
    if (ok) {
      ...
      if (newStreak > bestStreak) {
        ...
        setCookie('fundamentBestStreak', String(newStreak));
      }
    } else {
      setStreak(0);
    }
  ...
  const shownLabel = shownField === 'symbol' ? t('fundament.field.symbol', locale) : t('fundament.field.name', locale);
  const hiddenLabel = shownField === 'symbol' ? t('fundament.field.name', locale) : t('fundament.field.symbol', locale);
  ...
  return (
    <div className="flex-1 flex flex-col items-center px-4 sm:px-6 py-8">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-6">
        <h2 className="font-serif text-2xl font-bold text-text-brightest mb-1">{t('fundament.game.title', locale)}</h2>
        ...
        >
          ...
          <div className="p-5 sm:p-7">
            ...
            <div className="py-3.5 px-3 rounded-xl">
              ...
              {shownField === 'symbol' ? (
                <FundamentSymbol symbol={shownValue} className="text-gold-light" size={40} />
              ) : (
              ...
              {isChecked ? (
                <div>
                  ...
                  {!result && (
                    <div className="mt-1">
                      {shownField === 'name' ? (
                        <FundamentSymbol symbol={correctAnswer} className="text-success/80" size={28} />
                      ) : (
                ...
                shownField === 'name' ? (
                  <FundamentSymbol symbol={userAnswer} className="text-violet-light" size={40} />
                ) : (
      ...
      <FundamentSelectionModal
        isOpen={modalOpen}
        onClose={() => setModalOpen(false)}
        onSelect={v => { setUserAnswer(v); setModalOpen(false); }}
        title={shownField === 'symbol' ? t('fundament.modal.pickName', locale) : t('fundament.modal.pickSymbol', locale)}
